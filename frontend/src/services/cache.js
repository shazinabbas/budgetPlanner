...
import { mockCategories, mockPaymentMethods, mockIncomeData, mockBudgetLimits } from '../utils/mockData';

const DB_NAME = 'budgetPlannerDB';
const DB_VERSION = 1;

// Import defaults from single source of truth (mockData.js)
const defaultCategories = mockCategories;
const defaultPaymentMethods = mockPaymentMethods;
const defaultIncome = mockIncomeData;
const defaultBudgetLimits = mockBudgetLimits;
...
class CacheStore {
  ...
  updatePaymentMethods(methods) {
    ...
    // Persist asynchronously
    this.setSetting('paymentMethods', methods).then(() => {
      this.queueSync({
        type: 'UPDATE',
        collection: 'settings',
        key: 'paymentMethods',
